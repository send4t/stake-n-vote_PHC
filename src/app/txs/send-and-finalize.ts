...
export async function sendAndFinalize(config: {
  ...
  return new Promise<SendAndFinalizeResult>(async (resolve, reject) => {
    try {
      const unsub = await tx.signAndSend(
        ...
        (result: ISubmittableResult) => {
          ...
          if (status.isReady) {
            ...
            console.log(
              `Tranzakció bekerült a blokkba ${status.asFinalized}`
            )
            ...
            if (dispatchError) {
              if (dispatchError.isModule) {
                ...
                console.trace("küldési hiba", decoded)
              } else {
                res = {
                  status: "error",
      ...
      console.error("Hiba az összeg küldése közben:", e)
